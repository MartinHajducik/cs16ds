...
RUN dpkg --add-architecture i386 && apt update && apt install -y lib32gcc1 curl  && rm -rf /var/lib/apt/lists/*

# Install SteamCMD
RUN mkdir -p /opt/steam && cd /opt/steam && \
    curl -k -sqL "https://steamcdn-a.akamaihd.net/client/installer/steamcmd_linux.tar.gz" | tar zxvf - && \
    mkdir -p /opt/hlds  && \
...
RUN mkdir -p /opt/hlds/cstrike/addons/metamod/dlls && \
    curl -k -sqL "http://prdownloads.sourceforge.net/metamod/metamod-$metamod_version-linux.tar.gz?download" | tar -C /opt/hlds/cstrike/addons/metamod/dlls -zxvf -
